...
object GBTLearnerWithBagging extends Learner {
  private val ensemble: ArrayBuffer[GBTClassificationModel] = ArrayBuffer()
  ...
  def load_learner(): Unit = {
    ...
    for (dir <- model_dirs) {
      ...
      ensemble += GBTClassificationModel.load(dir.getPath)
    }
  ...
  def train(data: DataFrame): DataFrame = {
    ...
    // build the learner
    // Todo: Tune the gbt_classifier parameters.
    val gbt_classifier = new GBTClassifier()
    ...
    for (i <- bags.indices) {
      ...
      ensemble += gbt_classifier.fit(bags(i))
    }
